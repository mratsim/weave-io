...
# ------------------------------------------------
const
  WVIO_CacheLineSize* {.intdefine.} = 128
  ## Data structures accessed from multiple threads are padded by this value
  ## to avoid false sharing / cache thrashing.
  ## Most CPU Arch (x86 and ARM) are 64 bytes, but Intel prefetchers fetch 2 lines at once
  ## so 128 bytes is often necessary.
  MpscPadding = WVIO_CacheLineSize div 2
...
## - The data structure fits in 4x cache-line
...
template whenCount(body): untyped =
  when keepCount or defined(WVIO_THREADPOOL_DEBUG):
    body
...
proc send*[T, keepCount](chan: var ChannelMpscUnboundedBatch[T, keepCount], src: sink T) {.inline.}=
  ## Send an item to the back of the channel
  ## As the channel has unbounded capacity, this cannot fail
...
proc sendBatch*[T, keepCount](chan: var ChannelMpscUnboundedBatch[T, keepCount], first, last: sink T, count: SomeInteger) {.inline.} =
...
proc tryRecv*[T, keepCount](chan: var ChannelMpscUnboundedBatch[T, keepCount], dst: var T): bool =
  ...
  block:
    ...
    if not next.isNil:
      ...
      # Prefetch removed for Weave-IO (not critical for injector queue)
...
proc tryRecvBatch*[T, keepCount](chan: var ChannelMpscUnboundedBatch[T, keepCount], bFirst, bLast: var T, max: int32): int32 =
  ## Try receiving up to `max` items buffered in the channel
  ...
  while not next.isNil:
    ...
    # If we reach the `max` here, we bail out
    if result >= max:
